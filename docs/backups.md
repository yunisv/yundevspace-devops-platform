...
2btask бэкапится отдельно (`docker compose exec db pg_dump` + архив тома вложений — см. README самого репозитория `yunisv/2btask`). DefectDojo / Harbor, если поставите — тоже отдельно, у каждого свой встроенный механизм (это официальные установщики вне этого репозитория).
