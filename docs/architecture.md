...
| VPN / доступ | NetBird (self-hosted, отдельный сервер) | единственный путь к платформе снаружи | на WireGuard (быстрее и на порядок меньше кода, чем OpenVPN); control plane на отдельной от платформы машине, поэтому отказ/компрометация одного не затрагивает другое; цепляется к произвольному OIDC, можно подключить тот же Keycloak — см. `vpn-netbird.md` |
...
| Issue tracking / PM | 2btask (свой сервис) | задачи, трудозатраты, отделы, отчёты, выгрузка часов в 1С:УНФ | своя разработка под конкретный бизнес-процесс компании (списание часов в 1С, отделы/должности, чат) — то, что в GitLab CE и в готовых PM-инструментах не покрывалось один в один; SSO через тот же Keycloak, без второго провайдера (см. `adding-2btask.md`) |
