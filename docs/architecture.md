...
| VPN / доступ | NetBird (self-hosted) | единственный путь к платформе снаружи, вход через Keycloak | на WireGuard (OpenVPN в разы медленнее и на порядок больше кода); из mesh-решений единственное, что и self-hosted, и цепляется к произвольному OIDC — см. `vpn-netbird.md` |
