...
**Модель для роутера — `qwen2.5:3b-instruct-q4_K_M`, не 14B**, в отличие
от триажа находок DefectDojo. Классификация "какое из 4 действий" —
задача сильно проще, чем разбор находки безопасности, тяжёлая модель
тут не нужна и только добавляет задержку. Модель — отдельно от 14B,
нужно скачать (один раз): `docker exec -it devops-platform-ollama-1
ollama pull qwen2.5:3b-instruct-q4_K_M`. Раз `OLLAMA_MAX_LOADED_MODELS=1`
(см. `docs/local-llm.md`), при чередовании с триажем DefectDojo (14B)
будет разовая задержка на перезагрузку модели в память — не страшно,
это два разных сценария использования, редко пересекаются по времени.
