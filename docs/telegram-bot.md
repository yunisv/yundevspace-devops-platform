...
- Нажатие кнопки → **Answer Callback** (обязателен — иначе кнопка в
  ...
  (повторно проверяет права — на случай форварда/старой клавиатуры) →
  **Reply Callback Result**. Сейчас — просто заглушка-подтверждение,
  реальные действия подключаются в Этапе 4b.

**Роли по умолчанию** (правь `ROLE_ACTIONS` в нодах `Build Menu` и
`Handle Button` — таблица продублирована в обеих, синхронизировать
руками при изменении):
...
## Этап 4b (следующий): реальные действия

- **show_dojo_report** → **trigger_pipeline** → **create_task** — по
  очереди, в этом порядке (от простого к сложному, Plane API ещё не
  трогали в этом проекте). Подключаются и к LLM-пути (`Parse Route` →
  `action`), и к кнопочному (`Handle Button` → `action`) одним и тем же
  набором HTTP-запросов — деталь входа (голос/текст/кнопка) не должна
  влиять на то, что происходит дальше.
- `create_task` на кнопках — после нажатия понадобится ещё один шаг
  (выбор исполнителя кнопками из `USERS`, затем текст названия задачи)
  — многошаговый диалог, состояние между сообщениями хранить в workflow
  static data (как `armedUntil`/`armedBy` сейчас).
