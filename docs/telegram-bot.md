...
**Пока фото ИЛИ документ**, приложи не может нести Telegram-сообщение
+ голос/текст одновременно, поэтому лога такая: пока ждём название
(`awaitingTitleFor` взведён, после выбора исполнителя) — фото/документ
без сопровождения ставится как вложение (**одно** на задачу, MVP —
новое перетирает старое, без очереди из нескольких), а голос/текст всё
так же финализирует и создаёт задачу.
...
- **Is Task Attachment** → **Queue Attachment** — сохраняет
  `{fileId, fileName, mimeType}` в `pendingTasks[userId].attachment`,
  продлевает окно ожидания на 5 минут, отвечает подтверждением.
- После `Create Plane Task` — **Has Attachment?** → если есть:
  **Get Attachment File Path** → **Download Attachment** (тот же
  паттерн, что для голосовых) → **Upload Attachment to Plane** — одна
  код-нода на весь трёхшаговый flow (получить presigned upload →
  собрать raw multipart/form-data вручную с бинарником файла → залить
  → подтвердить `PATCH .../attachments/{asset_id}/`).
- **Код-нода не может использовать n8n credential напрямую** — только
  HTTP Request-ноды умеют. Поэтому в `Upload Attachment to Plane`
  токен Plane вписан явно в переменную `PLANE_TOKEN` (тот же токен,
  что в credential `Plane API`) — тот же паттерн, что уже был с
  токеном Telegram-бота в этом файле, не новый прецедент, но токен
  теперь буквально в тексте workflow, а не только в credential —
  учитывать при экспорте/шаринге файла воркфлоу.

**Обязательно перед использованием**: вписать `PLANE_TOKEN` в ноде
`Upload Attachment to Plane`.
