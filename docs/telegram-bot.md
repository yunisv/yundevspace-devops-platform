...
- **фото/документ без подписи** — просто вложение в очередь (**одно**
  на задачу, MVP — новое перетирает старое, без очереди из нескольких),
  ждём title отдельным сообщением;
- **голос/текст** — финализирует как обычно, без вложения (если оно не
  было отправлено раньше отдельным сообщением).
