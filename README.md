...
Записи указывают на **приватный** IP сервера — маршрут до него есть только
у тех, кто подключён по VPN.
...
8. **Закрыть публичный интерфейс**: `sudo ./scripts/harden.sh` — только после того, как проверили, что через VPN всё открывается ([docs/network-access.md](docs/network-access.md)).

## Доступ и сетевая безопасность

Платформа живёт в закрытом контуре. Вход — только через OpenVPN на
корпоративном файрволе (отдельная машина), дальше по приватной сети
Hetzner. Публичных портов у сервера платформы нет вообще: сервисы
привязаны к приватному интерфейсу (`BIND_ADDRESS`), поверх стоит ip-фильтр
Traefik, поверх — правила ufw. Сертификаты выпускаются через DNS-01, так
что входящие соединения не нужны даже для ACME.

Топология, настройка маршрутов и способ восстановления, если заперлись
снаружи, — в [docs/network-access.md](docs/network-access.md).
